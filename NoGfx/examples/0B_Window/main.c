...
void init(void) {
	...
	gGpuContext.window = RGFW_createWindow("Metal 4", 0, 0, 640, 480, RGFW_windowCenter);
	...
	surfaceDesc.size[1] = OUTPUT_HEIGHT;
	surfaceDesc.target.type = GPU_SURFACE_COCOA;
	surfaceDesc.target.cocoa.nsView = RGFW_window_getView_OSX(gGpuContext.window);

	gGpuContext.surface = gpuCreateSurface(&surfaceDesc, NULL);
...
void beginMainRenderpassOnFramebuffer(GpuCommandBuffer cb) {
	...
	frameBufferTarget.texture = gGpuContext.currentDrawable;
...
int main(void) {
	...
	while (!RGFW_window_shouldClose(gGpuContext.window)) {
		if (gGpuContext.onQueueDoneNext > 4) {
			gpuWaitSemaphore(gGpuContext.onQueueDoneNext, gGpuContext.onQueueDoneNext - 3, NULL);
		}

		int w, h;
		RGFW_window_getSize(gGpuContext.window, &w, &h);
		if (currHeight != h || currWidth != w) {
			currHeight = h;
			currWidth = w;

			gpuResizeSurface(gGpuContext.surface, (uint32_t[2]){ (uint32_t)w, (uint32_t)h }, NULL);
		}

		for (;;) {
			gGpuContext.currentDrawable = gpuAcquireNextDrawable(gGpuContext.surface, &result);
			if (result == GPU_SUCCESS) {
				break;
			}
		}

		cb = gpuStartCommandEncoding(gGpuContext.queue, NULL);
	...
	gpuWaitSemaphore(gGpuContext.onQueueDoneNext, gGpuContext.onQueueDoneNext, NULL);

	gpuDeinit();
	RGFW_window_close(gGpuContext.window);
